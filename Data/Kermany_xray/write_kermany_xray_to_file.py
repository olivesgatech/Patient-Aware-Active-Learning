...
img_path = '../../../../../../../OCT/BIGandDATA/ZhangData/chest_xray/'
file_path = './'
...
raw_tr = np.array([[np.array(Image.open(os.path.join(img_path, 'train', fname[0].split('-')[0],
                                                     '-'.join(fname[0].split('-')[1:])))),
                    int(fname[1]), int(fname[0].split('-')[2])]  # image, label, patient ID
                   for fname in train_list], dtype=object)
raw_te = np.array([[np.array(Image.open(os.path.join(img_path, 'test', fname[0].split('-')[0],
                                                     '-'.join(fname[0].split('-')[1:])))),
                    int(fname[1]), int(fname[0].split('-')[2])]  # image, label, patient ID
                   for fname in test_list], dtype=object)
